...
import { upsertLanguages } from '../../../src/language/repository/language.repository';
import {
  upsertUser,
  findUsers
...
describe('User service tests', () => {
  ...
  describe('Test of addUser method', () => {
    it('Should add a user and their languages successfully', async () => {
      ...
      (upsertUser as jest.Mock).mockResolvedValue(mockUserId);
      (getUserRepos as jest.Mock).mockResolvedValue(mockUserRepos);
      (upsertLanguages as jest.Mock).mockResolvedValue(undefined);
      ...
      expect(upsertLanguages).toHaveBeenCalledWith(mockUserId, [
        'JavaScript',
        'TypeScript'
      ]);
    });
